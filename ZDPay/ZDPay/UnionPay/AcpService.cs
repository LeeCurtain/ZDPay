...
namespace ZDPay.UnionPay
{
    public class AcpService
    {
        ...
        static AcpService()
        {
            log.info("false".Equals(SDKConfig.IfValidateRemoteCert) ? "不验https证书。" : "验https证书。");
        ...
        public static void SignByCertinfo(Dictionary<string, string> reqData, string certPath, string certPwd, Encoding encoding)
        {
            ...
            if ("01".Equals(signMethod))
            {
                ...
                string stringData = Util.CreateLinkString(reqData, true, false, encoding);
                ...
                if ("5.0.0".Equals(version))
                {
                    ...
                    string stringSignDigest = Util.ByteArray2HexString(signDigest);
        ...
        public static void SignBySecureKey(Dictionary<string, string> reqData, string secureKey, Encoding encoding)
        {
            ...
            {
                String strBeforeSha256 = stringData + "&" + Util.ByteArray2HexString(SecurityUtil.Sha256(secureKey, encoding));
                String strAfterSha256 = Util.ByteArray2HexString(SecurityUtil.Sha256(strBeforeSha256, encoding));
            ...
            {
                String strBeforeSm3 = stringData + "&" + Util.ByteArray2HexString(SecurityUtil.Sm3(secureKey, encoding));
                String strAfterSm3 = Util.ByteArray2HexString(SecurityUtil.Sm3(strBeforeSm3, encoding));
        ...
        public static bool Validate(Dictionary<string, string> rspData, Encoding encoding)
        {
            ...
            if ("01".Equals(signMethod))
            {
                ...
                if ("5.0.0".Equals(version))
                {
                    ...
                    string stringData = Util.CreateLinkString(rspData, true, false, encoding);
        ...
        public static bool ValidateBySecureKey(Dictionary<string, string> rspData, string secureKey, Encoding encoding)
        {
            ...
            if ("11".Equals(signMethod))
            {
                ...
                string strBeforeSha256 = stringData + "&" + Util.ByteArray2HexString(SecurityUtil.Sha256(secureKey, encoding));
                log.info("before final sha256: [" + strBeforeSha256 + "]");
                string strAfterSha256 = Util.ByteArray2HexString(SecurityUtil.Sha256(strBeforeSha256, encoding));
                result = stringSign.Equals(strAfterSha256);
                if (!result) log.info("after final sha256: [" + strAfterSha256 + "]");
            }
            else if ("12".Equals(signMethod))
            {
                string stringSign = rspData["signature"];
                ...
                log.info("待验签返回报文串：[" + stringData + "]");
                string strBeforeSm3 = stringData + "&" + Util.ByteArray2HexString(SecurityUtil.Sm3(secureKey, encoding));
                log.info("before final sm3: [" + strBeforeSm3 + "]");
                string strAfterSm3 = Util.ByteArray2HexString(SecurityUtil.Sm3(strBeforeSm3, encoding));
                result = stringSign.Equals(strAfterSm3);
                if (!result) log.info("after final sm3: [" + strAfterSm3 + "]");
            }
            else
            {
        ...
        public static bool ValidateAppResponse(string jsonData, Encoding encoding)
        {
            ...
            //获取签名
            Dictionary<string, object> data = Util.JsonToDictionary(jsonData);

            string stringData = (string)data["data"];
            string signValue = (string)data["sign"];
            Dictionary<string, string> dataMap = Util.parseQString(stringData, encoding);
        ...
        {
            string postData = Util.CreateLinkString(reqData, false, true, encoding);
            ...
            try
            {
                ...
                if (webResponse.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    return Util.CoverStringToDictionary(sResult, encoding);
                }
                else
                {
        ...
        {
            log.info("发送get请求：" + reqUrl); //get先info，缴费相关前端接口数据量貌似还挺大
            StreamReader reader = null;
            HttpWebResponse webResponse = null;
            try
            {
        ...
        public static string GetCustomerinfo(Dictionary<string, string> customerinfo, Encoding encoding)
        {
            ...
            string customerinfoStr = "{" + Util.CreateLinkString(customerinfo, false, false, encoding) + "}";
        ...
        public static string GetCustomerinfoWithEncrypt(Dictionary<string, string> customerinfo, Encoding encoding)
        {
            ...
            {
                string encryptedinfoStr = Util.CreateLinkString(encryptedinfo, false, false, encoding);
        ...
        public static Dictionary<string, string> ParseCustomerinfo(string customerinfoStr, string certPath, string certPwd, Encoding encoding)
        {
            ...
            Dictionary<string, string> customerinfo = Util.parseQString(s, encoding);
            if (customerinfo.Keys.Contains("encryptedinfo"))
            {
                ...
                Dictionary<string, string> encryptedinfo = Util.parseQString(encryptedinfoStr, encoding);
        ...
        public static int UpdateEncryptCert(Dictionary<string, string> dic, Encoding encoding)
        {
            ...
            if ("01".Equals(certType))
            {
                if (!CertUtil.GetEncryptCertId().Equals(x509Cert.SerialNumber.ToString()))
                {
                    ...
                    string newLocalCertPath = Util.GenBackupName(localCertPath);
